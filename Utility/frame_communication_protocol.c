...
__weak void FCP_Init( FCP_Handle_t * pHandle )
{
  ...
  pHandle->TxFrame.Head  = 0xffff;
  pHandle->TxFrame.ID = 0x0;
  ...
  pHandle->RxFrame.Head = 0x00;
  pHandle->RxFrame.ID = 0x0;
...
__weak uint8_t FCP_CalcCRC( FCP_Frame_t * pFrame )
{
  ...
  if( MC_NULL != pFrame )
  {
    ...
    for ( idx = 0; idx < pFrame->Size - 2; idx++ )
    {
...
{
  if ( MC_NULL != pFrame )
    return FCP_CalcCRC(pFrame) == pFrame->FrameCRC;
  else
    return 0;
}
